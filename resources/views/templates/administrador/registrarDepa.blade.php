	...
	<div class="limiter">
		<div class="container-login100" style="background-image: url('/plogin/images/img-01.jpg');">
			<div class="wrap-login100 p-t-10 p-b-5" style="width: 500px;">
				...
				<form action="{{route('registrarCambioD',[$dep->id_depat])}}" method="POST" class="login100-form validate-form" id="contactForm">
					{{ csrf_field() }}
					...
					<span class="login100-form-title p-t-10 p-b-5">
						Cambio del Jefe del {{$dep->nombre_depa}}
					</span>

					<font class="text-center w-full" color="Black" face="Comic Sans MS,arial,verdana" size="4">Profesor Actual 
 					 </font>
					<div  class="wrap-input100 validate-input m-b-10" data-validate = "Departamento is required" >
						
					
						<input value="{{$dep->nombre}}" readonly="readonly" class="input100" type="text" name="rol" >
					
					</div>

					<font class="text-center w-full" color="Black" face="Comic Sans MS,arial,verdana" size="4">Profesor Nuevo
 					 </font>
					<div  class="wrap-input100 validate-input m-b-10" data-validate = "Departamento is required" >
					
						<select required="" name="profesor" id="inputCategoria_id" class="input100">		

							<option value="">Seleccione un Profesor</option>
								@foreach ($usuarios as $usu)
								@if($usu->id != $usu->id_Jefe)
					...
					<div class="container-login100-form-btn p-t-10">
					<a href="<?php echo route('busqueda') ?>" class="login100-form-btn">Regresar
						<button></button></a>
