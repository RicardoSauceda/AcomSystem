  		...
  		<table class="table table-striped table-bordered table-hover table-condensed table-fixed">
		    ...
		    <tbody>
		    	...
		    	<tr>
		     	...
		     	{{csrf_field ()}}	
					<!--<td>	<select  required="" style="margin: 5px;" name="pos">
											<option value="1">Autorizar</option>
											<option value="0">No Autorizar</option>
							</select>
					</td>-->
					<td>
						<!--<button style="margin: 5px;" class="btn btn-success">ENVIAR</button>-->
						<a href="{{ route('AutorizarP',[$proyecto->id]) }}" class="btn btn-success"> AUTORIZAR</a>
					</td>
					<td>
						<a class="btn btn-danger" href="{{ route('NoAutorizarP',[$proyecto->id]) }}" onclick="return confirm('¿Estas seguro que deceas Rechazar la Actividad?')">RECHAZAR</a> 
					 	
					</td>
					<!--</form>
					
					<td>
						<a href="{{ route ('SolicitudesJefe', [$proyecto->codigo, $proyecto->id])}}" >Solicitudes para el Proyecto de {{ $proyecto->nombre}}</a>
					</td>
					<td>
						<a href="{{ route ('inscritos', [$proyecto->codigo, $proyecto->id])}}" >Aceptados al Proyecto de {{ $proyecto->nombre}}</a>
					</td>

					<td>
						 <a class="btn btn-info" href="{{route('PDF', [$proyecto->codigo])}}"> PDF lista de Inscritos</a>
					</td>
